...
class UltraVideoProcessor:
    ...
    def _get_track_velocity(track) -> tuple:
        if hasattr(track, 'get_velocity'):
            try:
                ...
                pass
        return float(getattr(track, 'vx', 0.0)), float(getattr(track, 'vy', 0.0))
    ...
    def process_frame_tracking_only(self, frame: np.ndarray, frame_timestamp: float = None) -> Tuple[np.ndarray, Dict]:
        """
        Draw tracker state only, without inference.
        Useful when the upstream loop chooses to skip detector work.
        """
        ...
        for track in confirmed_tracks:
            ...
            label = f"{class_name} ID:{track_id}{stale_suffix}"
